...
create table if not exists mutual_funds (
    ...
    scheme_type       text not null check (scheme_type in ('equity','debt','hybrid','liquid')),
    units             numeric(18,4) not null,
    avg_cost_nav      numeric(12,4) not null,
    ...
    source            text default 'unavailable' check (source in ('live','cached','unavailable')),
...
create table if not exists bonds (
    ...
    bond_type           text not null check (bond_type in ('gsec','corp','tax_free','perpetual')),
    face_value          numeric(12,2) not null,
    units               numeric(14,2) not null,
    coupon_rate         numeric(6,4) not null,
    coupon_freq         text not null check (coupon_freq in ('annual','semi')),
    maturity_date       date,
...
create table if not exists gold_holdings (
    id                       uuid primary key default gen_random_uuid(),
    client_id                uuid not null references clients(id) on delete cascade,
    gold_type                text not null check (gold_type in ('physical','sgb','etf','fund')),
...
create table if not exists fixed_deposits (
    ...
    client_id       uuid not null references clients(id) on delete cascade,
    bank_name       text not null,
    fd_number_last4 text,
    principal       numeric(14,2) not null,
    interest_rate   numeric(6,4) not null,
    compounding     text not null check (compounding in ('simple','monthly','quarterly','annual')),
    start_date      date not null,
    maturity_date   date not null,
    payout_type     text not null check (payout_type in ('cumulative','payout')),
    created_at      timestamptz default now()
);
...
create table if not exists insurance_policies (
    id                    uuid primary key default gen_random_uuid(),
    client_id             uuid not null references clients(id) on delete cascade,
    policy_type           text not null check (policy_type in ('term','endowment','ulip','health','whole_life')),
...
-- 10. Gold price cache (support) ─────────────────────────────────────
create table if not exists gold_price_cache (
    gold_type        text primary key,
